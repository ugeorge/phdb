...
class LaTeXOut():
	...
	def writeout(self, msg, variables):
		...
		if msg == 'reviews':
			...
			self.custom(variables)

	def reviews(self, variables):
		...
		rows = map(lambda x: ['' if v is None else v for v in list(x)], rows)
		writeTable(f, col_names, rows, self._widths, "Tab01")
		writefooter(f)
		f.close()
...
\usepackage[left=2cm,bottom=1cm,right=1cm, top=1cm, a4paper]{geometry}
\usepackage{booktabs}
...
def writeTable(f, headers, rows, widths, tabname):
	if len(headers) > len(widths):
		remaining = (1.0 - sum(widths)) / (len(headers) - len(widths))
		print sum(widths), (len(headers) - len(widths)), remaining
		if remaining < 0: raise Exception('IOException', 'Sum of widths > 1')
		widths = widths + (len(headers) - len(widths)) * [remaining,]
	elif len(headers) < len(widths):
		widths = widths[:len(headers)-1]

	text = '\\begin{longtable}{@{}' + \
			' | '.join(['p{' + str(x) + '\\textwidth}' for x in widths]) + \
			'@{}}\n'
	text += '\\toprule\n' + " & ".join(headers) + "\\\\\n" + "\\midrule\n\\endhead\n\n"
	f.write(text)
	for row in rows:
